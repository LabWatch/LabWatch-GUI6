...
class DrawGauge2(ini):
    ...
    def draw_tick(self,divisions=100):
        ...
        self.canvas.create_text(self.size/2,3*self.size/5, font=("Segoe UI",int(self.size/18),'bold'),fill="black", text=label,angle=0)
        self.readout = self.canvas.create_text(self.size/2,4*self.size/5, font=("Segoe UI",int(self.size/18),'bold'),fill="black", text='')
        ...
        for tick in range(divisions+1):
            ...
            if (tick%10) == 0:
                ...
                self.canvas.create_text(x,y, font=("Segoe UI",int(self.size/25)),fill="black", text=label)
    ...
    def set_value(self, number: (float, int)):
        ...
        self.readout = self.canvas.create_text(self.size/2,4*self.size/5, font=("Segoe UI",int(self.size/16),'bold'),fill="black", text=label,angle=0)
...
class DrawGauge3(ini):
    ...
    def draw_tick(self,divisions=100):
        ...
        self.canvas.create_text(self.size/2,3*self.size/5, font=("Segoe UI",int(self.size/20),'bold'),fill="black", text=label,angle=0)
        self.readout = self.canvas.create_text(self.size/2,4*self.size/5, font=("Segoe UI",int(self.size/18),'bold'),fill="black", text='')
        inner_tick_radius2 = int((self.size-self.size/9) * 0.48)
        outer_tick_radius2 = int((self.size-self.size/9) * 0.50)
        inner_tick_radius3 = int((self.size-self.size/9) * 0.35)
